...
jest.mock("../../js/users/account/purchaseHistory/purchaseHistory"); // remove on auto mock true
// jest.unmock("../../js/users/account/account"); // add on auto mock true
...
describe("createAccount", () => {
  let newEmailAddress = "newUser@newuser.com";
  beforeEach(() => {
    jest.spyOn(users, "userExists").mockResolvedValue(false);
    jest.spyOn(users, "createUserId").mockReturnValue(2);
  });

  test("Returns user data when account created successfully -Promises", () => {
    // remember to add the return to async functions
    // can add has assertions to confirm test has run and not gave a false positive

    expect.hasAssertions();

    return account.createAccount(newEmailAddress).then((userAccount) => {
      expect(userAccount.data).toStrictEqual({
        userId: 2,
  ...
  test("Returns user data when account created successfully - async/await alternative", async () => {
    // remember to add the return to async functions
    // can add has assertions to confirm test has run and not gave a false positive
  ...
  test("Returns user data when account created successfully - .resolves alternative", () => {
    // remember to add the return to async functions
    // can add has assertions to confirm test has run and not gave a false positive
  ...
  test("Returns error message when a user already exists .rejects", () => {
  ...
  test("Returns error message when a user already exists .rejects -async/await", async () => {
    jest.spyOn(users, "userExists").mockResolvedValue(true);
    expect.hasAssertions();
